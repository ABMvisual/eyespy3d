// --- 0. DYNAMIC AUDIO ENGINE & PRELOADER ---
...
function preloadLevelAudio(levelIndex, levelsArray) {
  const currentLevel = levelsArray[levelIndex];
  if (!currentLevel) return;

  currentLevel.imagesToFind.forEach(imageFilename => {
    let mp3Name = AUDIO_MAP[imageFilename];
    let audioUrl = mp3Name ? (GITHUB_BASE + encodeURIComponent(mp3Name)) : 'https://upload.wikimedia.org/wikipedia/commons/4/42/Ping.mp3';
  ...
  const chimePreloader = new Audio();
  chimePreloader.preload = "auto";
  chimePreloader.src = 'https://upload.wikimedia.org/wikipedia/commons/d/d7/Tada.mp3';
}
...
function startMechanics() {
  ...
  // LIGHTWEIGHT VISUAL HUNTER
  setInterval(() => {
    document.querySelectorAll('[class*="close"], [id*="close"]').forEach(btn => {
      const rect = btn.getBoundingClientRect();
      if (rect.bottom > window.innerHeight - 100) { btn.style.setProperty('display', 'none', 'important'); btn.style.setProperty('opacity', '0', 'important'); }
    });
    ...
    // Restricted query to prevent lag
    const textElements = document.querySelectorAll('h1, h2, h3, p, span, [class*="title"], [class*="content"]');
  ...
  window.currentLevelIndex = 0; window.allModelSweeps = []; window.foundImages = {}; window.isTeleporting = false; window.pathsPreloaded = false; window.activeOpenPopups = new Set(); 
  ...
  function setupLevelTracking() {
    ...
    window.isTeleporting = false; window.pathsPreloaded = false; window.activeOpenPopups.clear();
    preloadLevelAudio(window.currentLevelIndex, LEVELS);
  }

  function checkAllFound() { return Object.values(window.foundImages).every(status => status === true); }

  // LIGHTWEIGHT TRIPWIRE (No HTML Stringification)
    ...
    mutations.forEach((mutation) => {
      mutation.addedNodes.forEach((node) => {
        if (node.nodeType === 1 || node.nodeType === 3) { 
          ...
          if (node.nodeType === 1) { // Extract image sources cleanly
            const media = node.tagName === 'IMG' ? [node] : node.querySelectorAll('img, [style*="background-image"]');
            media.forEach(m => searchString += ' ' + (m.src || m.style.backgroundImage || '').toLowerCase());
          }
          ...
          currentLevel.imagesToFind.forEach((filename) => {
            ...
            if (searchString.includes(cleanName) || searchString.includes(encodedName)) {
              ...
              window.activeOpenPopups.add(filename); window.foundImages[filename] = true;
