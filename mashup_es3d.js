// =============================================================================
// EYE SPY 3D — V500 EXPANDED: THE PASSIVE SCANNER BASELINE + LEVELS 8-14
// =============================================================================
...
function injectCustomUI() {
  ...
  // SECONDARY JS AUDIO ASSASSIN (Throttled)
  setInterval(() => {
...
function startMechanics() {
  ...
  // --- THE PASSIVE VISUAL HUNTER (Replaces MutationObserver) ---
  setInterval(() => {
    const currentLevel = LEVELS[window.currentLevelIndex];
    if (!currentLevel || currentLevel.imagesToFind.length === 0 || window.isTeleporting) return;

    // Only scan specific MPEmbed popup containers
    ...
    popups.forEach(popup => {
        ...
        currentLevel.imagesToFind.forEach(filename => {
            ...
            if (searchString.includes(cleanName) || searchString.includes(encodedName)) {
                ...
                // Apply massive text styling instantly
  ...
  }, 250); // Wakes up safely every 250ms
